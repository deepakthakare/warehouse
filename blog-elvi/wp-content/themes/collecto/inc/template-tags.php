...
function collecto_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . '<span>' . esc_html__( 'Tags: ', 'collecto' ) . '</span>' .  $tags_list . '</span>' ); // WPCS: XSS OK.
		}
...
function collecto_footer_widgets() {
	...
	foreach ( $footer_sidebars as $footer_sidebar ) {
		...
		if ( is_active_sidebar( $footer_sidebar ) ) { ?>
			...
			<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6 widget-area" style="padding: 0 10px;">
...
function collecto_numbers_pagination() {
	...
	/* instagram footer widgets dispaly here */
