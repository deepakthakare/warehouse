...
/** @var string Location */
	...
	public static $definition = array(
		...
		'fields' => array(
			...
			'upc' => 						array('type' => self::TYPE_STRING, 'validate' => 'isUpc', 'size' => 30),

			'lm_season' => 					array('type' => self::TYPE_STRING, 'validate' => 'isCleanHtml'),

			'lm_location' => 				array('type' => self::TYPE_STRING, 'validate' => 'isCleanHtml'),
	...
	**/
...

	{
		...
		//Set By Ranjit Das to create new price display rule by using default rule settings rules

		// reference parameter is filled before any returns

		$specific_price_rule = SpecificPrice::getSpecificPriceRule(
		...
		// Special Ruel Set modified/Added by Ranjit Das 2015-03-25
...

		{
			...
			if ($specific_price_rule['reduction_type'] == 'amount')

			{

				$reduction_amount_rule = $specific_price_rule['reduction'];



				if (!$specific_price_rule['id_currency'])

					$reduction_amount_rule = Tools::convertPrice($reduction_amount_rule, $id_currency);

				$priceDiferRule = Tools::ps_round($reduction_amount_rule, $decimals);

			}

			else

				$priceDiferRule = Tools::ps_round($price * $specific_price_rule['reduction'], $decimals);

		}

		

		$price += $priceDiferRule;
	...
	*/
...

	{
		...
		$sql->select('p.`id_product`, pl.`name`, pl.`custom_field`, p.`ean13`, p.`upc`, p.`active`, p.`reference`, m.`name` AS manufacturer_name, stock.`quantity`, product_shop.advanced_stock_management, p.`customizable`');
		...
		OR pl.`custom_field` LIKE \'%'.pSQL($query).'%\'
